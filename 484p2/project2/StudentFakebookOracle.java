...
public final class StudentFakebookOracle extends FakebookOracle {
    ...
    public FirstNameInfo findNameInfo() throws SQLException {
        try (Statement stmt = oracle.createStatement(FakebookOracleConstants.AllScroll, FakebookOracleConstants.ReadOnly)) {
            ...
            //get the shortest first names
            rst = stmt.executeQuery(
                "SELECT DISTINCT First_Name " +
                "FROM " + UsersTable + " " +
                "GROUP BY First_Name " +
                "HAVING LENGTH(First_Name) = (SELECT MIN(LENGTH(First_Name)) FROM " + UsersTable + ") " +
                "ORDER BY First_Name"
            );
            ...
            //get the most common first name and its/their count
            rst = stmt.executeQuery(
                "SELECT DISTINCT First_Name, COUNT(First_Name) " +
                "FROM " + UsersTable + " " +
                "GROUP BY First_Name " +
                "HAVING COUNT(First_Name) = (SELECT MAX(COUNT(First_Name)) FROM " + UsersTable + " GROUP BY First_Name) " +
                "ORDER BY First_Name"
            );
